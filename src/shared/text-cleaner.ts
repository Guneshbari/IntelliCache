...
export function deduplicateRepeatedPhrase(text: string): string {
  ...
  const match = text.match(/^(.{3,}?)(?:(?:[\s,.;:!?]+)\1)+$/i)
  if (match) {
    const candidate = match[1].trim()
    if (/\s/.test(candidate) || candidate.length > 10) {
      return candidate
    }
  }
  return text
...
export function cleanQueryText(raw: string): string {
  ...
  text = text.replace(/^\s*(?:you\s+said|user|human)\s*[:：\-–—]?\s*/i, '').trim()
...
export function cleanResponseText(raw: string): string {
  ...
  text = text.replace(/^\s*(?:chatgpt|claude|gemini|assistant)\s*said\s*[:：\-–—]?\s*/i, '').trim()
